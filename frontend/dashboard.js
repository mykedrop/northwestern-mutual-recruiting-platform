...
class Dashboard {
    ...
    async init() {
        // Check authentication - PRODUCTION MODE
    ...
    initWebSocket() {
        this.socket = io('window.API_BASE_URL || ''', {
    ...
    async loadPipelineData(forceRefresh = false) {
        try {
            console.log('Loading pipeline data...');

            // Sample pipeline data - in a real app this would come from API
            const pipelineData = {
                'pipeline-new': [
                    { id: 1, name: 'John Smith', role: 'Financial Advisor', score: 92, avatar: 'JS' },
                    { id: 2, name: 'Sarah Johnson', role: 'Insurance Agent', score: 88, avatar: 'SJ' },
                    { id: 3, name: 'Michael Brown', role: 'Wealth Manager', score: 91, avatar: 'MB' }
                ],
                'pipeline-assessment': [
                    { id: 4, name: 'Mike Chen', role: 'Wealth Manager', score: 85, avatar: 'MC' },
                    { id: 5, name: 'Lisa Wang', role: 'Financial Advisor', score: 90, avatar: 'LW' }
                ],
                'pipeline-interview': [
                    { id: 6, name: 'David Brown', role: 'Regional Manager', score: 94, avatar: 'DB' }
                ],
                'pipeline-review': [
                    { id: 7, name: 'Emma Davis', role: 'Financial Advisor', score: 89, avatar: 'ED' }
                ],
                'pipeline-hired': [
                    { id: 8, name: 'Alex Wilson', role: 'Insurance Agent', score: 96, avatar: 'AW' }
                ]
            };

            // Populate each pipeline column
            Object.keys(pipelineData).forEach(columnId => {
                const column = document.getElementById(columnId);
                if (column) {
                    column.innerHTML = pipelineData[columnId].map(candidate => `
                        <div class="pipeline-card" draggable="true" data-candidate-id="${candidate.id}">
                            ...
                            <div class="candidate-actions">
                                <button class="btn-action" onclick="dashboard.viewCandidate(${candidate.id})">View</button>
                                <button class="btn-action" onclick="dashboard.contactCandidate(${candidate.id})">Contact</button>
                            </div>
                        </div>
                    `).join('');
                }
    ...
    initializeDragAndDrop() {
        ...
        columns.forEach(column => {
            ...
            column.addEventListener('drop', (e) => {
                ...
                const draggedElement = document.querySelector(`[data-candidate-id="${candidateId}"]`);

                if (draggedElement && draggedElement.parentNode !== column) {
                    // Move the card to the new column
                    column.appendChild(draggedElement);
                    console.log(`Moved candidate ${candidateId} to ${column.id}`);

                    // Update backend (placeholder for now)
                    this.updateCandidateStage(candidateId, column.id.replace('pipeline-', ''));
                }
            });
        });
    }
    ...
    async updateCandidateStage(candidateId, newStage) {
        try {
            ...
            this.showSuccessMessage(`Candidate moved to ${newStage} stage`);
        } catch (error) {
            console.error('Error updating candidate stage:', error);
            this.showErrorMessage('Failed to update candidate stage');
        }
    }
    ...
    switchView(view) {
        ...
        if (view === 'candidates') {
            ...
            this.loadPipelineData(true); // Force refresh
        }
    }
    ...
    async checkExportStatus(exportId) {
        try {
            const response = await fetch(`window.API_BASE_URL || ''/api/export/status/${exportId}`, {
                headers: {
