...
const authMiddleware = async (req, res, next) => {
    try {
        ...
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
            // ONLY in development, use a fallback user
            if (process.env.NODE_ENV !== 'production') {
                const fallbackUserId = process.env.DEV_RECRUITER_ID || 'f52e6779-a3bc-4895-a94d-5c6540c2d2c6';

                // Load user context even for fallback
